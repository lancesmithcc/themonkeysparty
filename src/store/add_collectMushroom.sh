...
# Make a backup first
cp src/store/gameStore.ts src/store/gameStore.ts.bak

# Create a temporary file with the collectMushroom implementation
cat > /tmp/collectMushroom.txt << 'EOF'
...
# Replace the line with setSecondNpcHit and everything after it with our new implementation
sed -i.old "19105,\$s/setSecondNpcHit: () => set({ isSecondNpcHit: true, secondNpcHitTime: Date.now() }).*$/$(cat /tmp/collectMushroom.txt)/g" src/store/gameStore.ts 